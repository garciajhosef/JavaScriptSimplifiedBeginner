...
function setup(onDragComplete) {
  (0, _addGlobalEventListener.default)("mousedown", "[data-draggable]", function (e) {
    var selectedItem = e.target.closest("[data-draggable]");
...
function setupDragEvents(selectedItem, itemClone, ghost, offset, onDragComplete) {
  ...
  document.addEventListener("mouseup", function () {
    ...
    if (dropZone) {
      ...
      onDragComplete(_objectSpread(_objectSpread({}, dragComplete), {
        index: Array.from(dropZone.children).indexOf(selectedItem)
      }));
      console.log(Array.from(dropZone.children).indexOf(selectedItem));
    } else {
      stopDrag(selectedItem, itemClone, ghost);
      console.log(Array.from(dropZone.children).indexOf(selectedItem));
    }
...
var lanes = loadLanes();
var formAddLane = document.querySelector("#form-add-lane");
var lanesContainer = document.querySelector(".lanes");
var laneTemplate = document.querySelector("#lane-template");
var taskTemplate = document.querySelector("#task-template"); //createLaneElement("ASSASA");
...
(0, _addGlobalEventListener.default)("submit", "[data-task-form]", function (e) {
  ...
  var laneElement = e.target.closest("[data-lane-id]");
  var tasks = laneElement.querySelector(".tasks");
...
(0, _addGlobalEventListener.default)("click", "[data-delete-lane]", function (e) {
  deleteLane(e);
});
(0, _addGlobalEventListener.default)("click", "[data-delete-task]", function (e) {
  deleteTask(e);
...
function onDragComplete(e) {
  console.log(e);
  var startLaneId = e.startZone.closest("[data-lane-id]").dataset.laneId;
  var endLaneId = e.endZone.closest("[data-lane-id]").dataset.laneId;
...
function renderLanes() {
  Object.entries(lanes).forEach(function (obj) {
    var laneId = obj[0];
    var laneElement = createLaneElement(laneId);
    lanesContainer.appendChild(laneElement);
    obj[1].forEach(function (task) {
...
function renderTaskFromLane(task, laneElement) {
  var tasksContainer = laneElement.querySelector(".tasks");
  var taskElement = createTaskElement(task);
  tasksContainer.append(taskElement);
}
...
function renderTasks() {
  Object.entries(lanes).forEach(function (obj) {
    var laneId = obj[0];
    var tasks = obj[1];
    var laneContainer = document.querySelector("[data-lane-id=\"".concat(laneId, "\"]")); //console.log(obj);

    var tasksContainer = laneContainer.querySelector(".tasks");
...
function createTaskElement(task) {
  ...
  // return element;
  var element = taskTemplate.content.cloneNode(true);
  var taskElement = element.querySelector(".task");
  taskElement.id = task.id;
  taskElement.dataset.draggable = true;
  var taskName = taskElement.querySelector("[data-task-name]");
  taskName.innerText = task.text;
  return taskElement;
}

function createLaneElement(lane) {
  var laneTemplateClone = laneTemplate.content.cloneNode(true);
  var laneElement = laneTemplateClone.querySelector(".lane");
  laneElement.dataset.laneId = lane;
  var laneHeader = laneElement.querySelector(".header");
...
function deleteLanes() {
  ({}), _readOnlyError("lanes");
  saveLanes();
}

formAddLane.addEventListener("submit", function (e) {
  e.preventDefault();
  var laneName = e.target.querySelector("#lane-name").value;
  if (laneName == "") return;
  var laneElement = createLaneElement(laneName);
  lanesContainer.appendChild(laneElement);
  lanes[laneName] = [];
  saveLanes();
});
...
function deleteTask(e) {
  var taskElement = e.target.closest(".task");
  var laneElement = e.target.closest("[data-lane-id]");
  var laneID = laneElement.dataset.laneId;
  lanes[laneID] = lanes[laneID].filter(function (task) {
    return task.id !== taskElement.id;
  });
  console.log(lanes);
  saveLanes();
  taskElement.remove();
